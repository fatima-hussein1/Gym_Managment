...
Public Class FrmLogin
    ...
    Private Sub FrmLogin_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Catch ex As Exception
            ShowAppMessage("ÝÔá Ýí ÌáÈ ÈíÇäÇÊ ÇáãÓÊÎÏãíä Ãæ ÇáÅÊÕÇá: " & ex.Message, AppMessageType.Error)
        Finally
            Try
                CloseCon()
            Catch
    ...
    Private Sub BtnOK_Click(sender As Object, e As EventArgs) Handles BtnOK.Click
        ...
        Try
            OpenCon()
        Catch ex As Exception
            ShowAppMessage("ÝÔá Ýí ÝÊÍ ÇáÇÊÕÇá: " & ex.Message, AppMessageType.Error)
            Exit Sub
        End Try
    ...
    WHERE UserName = @u AND UserPass = @p AND LockedOut = 0", con)
                ...
                Using rdr As SqlDataReader = cmd.ExecuteReader()
                    If rdr.Read() Then
                        If Not rdr.IsDBNull(rdr.GetOrdinal("LockedOut")) Then
                            ...
                            If locked Then
                                ShowAppMessage("ÇáÍÓÇÈ ãÞÝá. ÊæÇÕá ãÚ ÇáãÓÄæá.", AppMessageType.Warning)
                                TxtPasswprd.Text = ""
                                TxtPasswprd.Focus()
                                Return
                            End If
            ...
            If authenticated Then
                Try
                    ...
                    End Using
                Catch ex As Exception
                    ShowAppMessage("ÎØÃ Ýí ÊÍãíá ÕáÇÍíÇÊ ÇáãÓÊÎÏã: " & ex.Message, AppMessageType.Error)
                End Try
            ...
            End If
        Catch ex As Exception
            ShowAppMessage("ÎØÃ  ÇÊÕÇá ÈÞÇÚÏÉ ÇáÈíÇäÇÊ: " & ex.Message, AppMessageType.Error)
        Finally
            Try
                CloseCon()
            Catch
